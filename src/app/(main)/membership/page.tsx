...
export default function MembershipPage() {
  ...
  const contentRef = useMemoFirebase(() => firestore ? doc(firestore, 'page_content', 'membership') : null, [firestore]);
        ...
        <div className="mt-16 grid grid-cols-1 gap-12 md:grid-cols-5">
          ...
          <div className="md:col-span-2">
            <Card className="bg-secondary sticky top-24">
              <CardHeader>
                {isLoading ? <Skeleton className="h-8 w-1/2" /> : <CardTitle className="font-headline text-2xl text-primary">{t.applyTitle}</CardTitle>}
              </CardHeader>
              <CardContent>
                {isLoading ? <Skeleton className="h-12 w-full" /> : <p className="text-muted-foreground mb-6">{t.applyDescription}</p>}
